...
def render_dashboard(file):
    ...
    st.title(f"Dashboard pogodowy")

    # Sidebar
    st.sidebar.header("Modyfikuj widok")
    show_table = st.sidebar.checkbox("Pokaz tabele z danymi", value=False)
    ...
    if df.empty:
        st.warning("Brak danych do wyswietlenia")
    ...
    with stats_cols[1]:
        if "humidity" in df.columns:
            stats_cols[1].warning(
                f"Srednia wilgotnosc {df['humidity'].mean():.2f}% \n\n"
    ...
    with stats_cols[2]:
        if "pressure" in df.columns:
            stats_cols[2].error(
                f"Srednie cisnienie {df['pressure'].mean():.2f}hPa \n\n"
    ...
    with stats_cols[3]:
        if "wind" in df.columns:
            stats_cols[3].info(
                f"Srednia predkosc wiatru {df['wind'].mean():.2f}km/h \n\n"
                f"Minimalna {df['wind'].min():.2f}km/h \n\n"
                f"Maksymalna {df['wind'].max():.2f}km/h \n\n"
            )
    ...
    st.subheader("Ostatnie 10 pomiarow")
